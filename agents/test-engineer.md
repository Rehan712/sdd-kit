...
| `loopback4` | mocha + `@loopback/testlab` (the LB4 scaffold default) | testlab `createRestAppClient` against a sandbox Postgres / sqlite | n/a (covered by frontend e2e) |
...
- Mock something the test claims to integrate. The rule: mock only at the
  boundary the test is NOT about — MSW/aws-sdk-client-mock stand in for
  *third-party* seams while your own DB/handler/route runs real; an
  "integration test" whose own subject is mocked is a unit test wearing a
  costume. Integration tests hit the real DB (or a sandbox); HTTP routes hit
  the real handler.
