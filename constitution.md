...
5. **One worktree per spec.** Implementation runs on branch `spec/NNN-slug`, cut from the project's **base branch** (`.specify/stack.yml` `base_branch:`, default `dev`) into a sibling worktree via `scripts/spec-worktree.sh`. The base checkout stays clean; the PR (via `scripts/spec-pr.sh`) merges the branch back. Spec docs are authored/committed on the base branch before the branch is cut.
...
6. **Deterministic checks run as scripts, not model judgment.** AC coverage, ref integrity, gate presence, drift between skill copies — `sdd-analyze.sh`, `sync.sh --check`, `sdd-doctor.sh` decide these, cheaply and reproducibly. Model judgment is reserved for what actually needs it (design, adversarial review).
...
8. **Evidence, not claims.** A task checkbox is ticked only together with an `*Evidence:*` line quoting the acceptance command and its key output. Gates re-run evidence rather than trust it; a `[x]` with no evidence is treated as not done.
